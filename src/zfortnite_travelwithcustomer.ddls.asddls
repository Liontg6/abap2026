...
define view entity ZFORTNITE_TravelWithCustomer
  as select from ZFORTNITE_Travel                 as t

    inner join   ZFORTNITE_Customer               as c on t.CustomerId = c.CustomerId

    inner join   DDCDS_CUSTOMER_DOMAIN_VALUE_T(
                   p_domain_name : '/DMO/STATUS') as d on  d.value_low = t.Status
